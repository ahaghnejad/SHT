...
p = size(x,2);                            % p denotes the number of regressors (slope coefficients)
%
X = cell(1,N);
Y = cell(1,N);
for i = 1:N
...
sigsq_hat = zeros(1,N);
for i = 1:N
    sigsq_hat(i) = ((Y{i}-(X{i}*betahat_ols{i}))'*M*(Y{i}-(X{i}*betahat_ols{i})))/(T-p-1);  % The error variances for the individual units based on the OLS estimator
end
L1 = cell(1,N);
L2 = cell(1,N);
for i = 1:N
...
delta = (N^(1/2))*(((N^-1)*swamytilde-p)/((2*p)^(1/2)));    % Delta statistic proposed by Pesaran and Yamagata (2008)
EZ = p;
VZ = (2*p)*(T-p-1)/(T+1);
